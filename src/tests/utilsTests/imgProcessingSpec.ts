...
describe('resizeImage', () => {
  ...
  it('should return an error if the provided width or height is negative or 0', () => {
    expect(() => resizeImage(testImg, -300, 200)).toThrowError(
      'width and height should be positive numbers and greater then 0'
    );
    expect(() => resizeImage(testImg, 300, 0)).toThrowError(
      'width and height should be positive numbers and greater then 0'
    );
  });
